...
func (s *Searcher) processFile(filename string, mu *sync.Mutex, wg *sync.WaitGroup, ch chan error) {
	...
	// Так как разбиение происходит по пробельным символам, некоторые слова выделяются с прилегающими знаками препинания, в примерах есть такие слова как "Дети," или "(так".
	// Можно оставить в словах только буквы, используя регулярку или проверяя символы в слове методом unicode.IsLetter().
	// Однако напрямую из текста задания такое требование не следует, потому позволю себе считать знаки препинания частью ключевого слова.
